 ...
 * __Shards Part Of Pipeline Running In Parallel__. In both cases, the pipeline's `shardsPart` executes on multiple shards, as indicated in the shards array field at the base of the explain plan. In the first example, the aggregation runtime targets only three shards. However, in the second example, the runtime must broadcast the pipeline's `shardsPart` to run on all shards - the section "_Execution Of The Shards Part Of The Split Pipeline_" in this chapter discusses why.
...
All the recommended aggregation optimisations outlined in the [Pipeline Performance Considerations](../guides/performance.md) chapter equally apply to a sharded cluster. In fact, in most cases, these same recommendations, repeated below, become even more critical when executing aggregations on sharded clusters: 
 ...
 2. __Sorting - Use Limit With Sort__. The runtime has to transfer fewer records over the network, from each shard performing the _shards part_ of a split pipeline to the location that executes the pipeline’s _merger part_.
 
 3. __Sorting - Reduce Records To Sort__. If you cannot adopt point 1 or 2, then by moving a `$sort` stage to as late as possible in the pipeline, less memory pressure will occur on each shard performing the _shards part_ of the split pipeline. The runtime will stream fewer records over the network to the _merger part_ location.
 ...
 5. __Grouping - Group Summary Data Only__. The runtime has to move fewer records over the network from each shard performing the _shards part_ of a split pipeline to the _merger part_’s location.
